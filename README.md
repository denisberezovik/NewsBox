# NewsBox

About: NewsBox analyses millions of articles every day to deliver you access to the top trending news stories influencing the
world right now.

***This application features a news display based on the selected category, a news search, and the saving of news to Bookmarks via CoreData. Also, there is news feed pagination, but unfortunately due to the free api the number of pages is limited.***

**HomeScreen with selected cathegory "technology"**

<img src="https://user-images.githubusercontent.com/92160205/226643632-dece31e8-389d-4f42-8ee6-627ddc02bcdb.jpg" width=30% height=30%>

**SearchViewController** 

<img src="https://user-images.githubusercontent.com/92160205/226644483-a242ddde-efe0-403f-b417-ceea3f63f822.jpg" width=30% height=30%>

**Bookmarks with CoreData**

<img src="https://user-images.githubusercontent.com/92160205/226644962-d2410274-7b66-43f4-87e4-fc981e67acd3.jpg" width=30% height=30%>

[Screen Recording](https://youtu.be/_N6CUDu_VBk)

## Technologies and tools
* Frameworks: UIKit, Foundation, CoreData, userDefaults, SafariServices, LocalAuthentication
* TableView, CollectionView, UITabBarController, UISearchController, CoreDataService, NSCache
* Using the MVC architecture
* Work with Api
* Designing an interface using code

## News API
* [NewsAPI](https://newsapi.org) - News API

## Contact Info
* [Linkedin](https://www.linkedin.com/in/denis-berezovik/) 

denisberezovik@gmail.com
